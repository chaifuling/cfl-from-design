...
const layouts = {
  colFormItem(h, scheme) {
    ...
    return (
      <a-form-item prop={scheme.__vModel__}
        ...
        if (Array.isArray(config.regList)) {
          if (config.required) {
            ...
            if (Array.isArray(config.defaultValue)) {
              required.type = 'array'
              required.message = `${this.tFn('base.select.least.one')}${config.label}`
            }
            required.message === undefined && (required.message = `${config.label}${this.tFn('base.not.null')}`)
