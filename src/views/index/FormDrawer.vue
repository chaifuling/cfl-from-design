    ...
    const clipboard = new ClipboardJS('.copy-btn', {
      text: trigger => {
        ...
        this.$notification.success({
          message: this.tFn('base.successfully'),
          description:
            this.tFn('base.code.copy.successfully')
        })
        return codeStr
      }
    })
    clipboard.on('error', e => {
      this.$message.error(this.tFn('base.code.copy.failed'))
    })
  },
  beforeDestroy() {
  ...
  methods: {
    ...
    runCode() {
      ...
      try {
        ...
        if (astBody.length > 1) {
          this.$confirm(
            this.tFn('base.js.format.failed'),
            this.tFn('base.hint'),
            {
              type: 'warning'
            }
        ...
        if (astBody[0].type === 'ExportDefaultDeclaration') {
          ...
          this.$message.error(this.tFn('base.use.export.export'))
        }
      } catch (err) {
        this.$message.error(`${this.tFn('base.use.export.export')}: ${err}`)
        console.error(err)
      }
    },
    generateCode() {
